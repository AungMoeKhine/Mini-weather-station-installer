...
volatile bool pendingMqttPublish = false;  // Tells Core 0 to send data safely
...
void waitForTimeSync() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    // Mutex Lock for License Initialization
...
void monitorSensors() {
  ...
  if (!upperSensor.isBusy() && now - lastScan >= ULTRASONIC_INTERVAL) {
    ...
    // Mutex Lock for writing sensor data to shared Config
    if (xSemaphoreTakeRecursive(systemMutex, portMAX_DELAY)) {
      if (dist > 0) {
...
void setup() {
  ...
  // Create the Recursive Mutex for thread safety
...
// ----------------------------------------------------------------------------
void networkTask(void* parameter) {
  static unsigned long lastPublish = 0;  // Rate limiter timer
  ...
  while (true) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      if (!mqttClient.connected()) {
        ...
        // NEW: Safely publish data only on Core 0, max once every 500ms
        if (pendingMqttPublish && (millis() - lastPublish >= 500)) {
          pendingMqttPublish = false;  // Lower the flag
          publishState();
          lastPublish = millis();
        }
      }
    ...
    // Auto-publish every 30 seconds as a heartbeat
    ...
    // Increased yield to give WiFi driver more stability
...
String processManualToggle() {
  ...
  // Mutex Lock: Modifying state variables
  if (xSemaphoreTakeRecursive(systemMutex, portMAX_DELAY)) {
    if (isSystemExpired) {
      xSemaphoreGiveRecursive(systemMutex);
  ...
  // Safe to call these outside lock
  updatePumpLogic();
  pendingMqttPublish = true;
  return res;
}
...
void updateLEDStatus() {
  ...
  // Grab lock quickly to read current state cleanly
  PumpState tState;
  bool tRunning;
  if (xSemaphoreTakeRecursive(systemMutex, portMAX_DELAY)) {
    tState = currentState;
...
void updatePumpLogic() {
  ...
  // Mutex Lock for entire logic processing
  if (!xSemaphoreTakeRecursive(systemMutex, portMAX_DELAY)) return;

  if (isSystemExpired && pumpConfig.motorStatus == 1) {
    pumpConfig.motorStatus = 0;
    saveMotorStatus();
    // We will publish state outside the lock below
  }
  ...
  bool triggerPublish = false;  // Flag to know if we should publish later
    ...
    if (currentMillis - voltageConfig.lastCheck >= GENERAL_INTERVAL) {
      ...
      if (voltageConfig.waitSeconds <= 0) {
        ...
        if (pumpConfig.wasRunningBeforeVoltageError) {
          if (tankConfig.rawUpperPercentage < tankConfig.FULL_THRESHOLD && !sensorError && !isSystemExpired) {
    ...
    case PumpState::DRY_RUN_LOCKED:
      ...
      if (dryRunConfig.autoRetryMinutes > 0 && currentMillis - dryRunConfig.lastRetryUpdate >= 1000) {
        ...
        if (dryRunConfig.retryCountdown <= 0) {
          ...
          bool safetyLock = (tankConfig.rawUpperPercentage >= tankConfig.FULL_THRESHOLD) || (tankConfig.upperInvalidCount >= TankConfig::MAX_INVALID_COUNT) || (voltageConfig.status == 0) || isSystemExpired;
          if (!safetyLock) {
            pumpConfig.motorStatus = 1;
            saveMotorStatus();
          }
  ...
  // MUST release the mutex before returning/publishing
  xSemaphoreGiveRecursive(systemMutex);

  // Instead of Core 1 publishing directly, raise a flag so Core 0 does it safely
  if (triggerPublish) {
    pendingMqttPublish = true;
  }
}
...
void updateLCD() {
  ...
  // Grab the lock to copy everything safely to local variables
  int val = 0, vVal = 0;
  ...
  // --- Draw outside the lock to keep screen slow-downs from blocking ---
...
bool reconnectMQTT() {
  ...
  if (isConnected) {
    ...
    pendingMqttPublish = true;  // Tell Core 0 to push initial state
    return true;
  } else {
...
void handleSettings() {
  ...
  // Mutex Lock for Safe UI Population
  if (xSemaphoreTakeRecursive(systemMutex, portMAX_DELAY)) {
